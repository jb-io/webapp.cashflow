...
export default function RuleEditor({ entry, apply }) {
  const rule = entry.rule;
  const setRule = (patch) => apply((draft) => { Object.assign(draft.rule, patch); });

  /** Typwechsel verwirft die typabhängigen Felder, behält aber die Ausnahmen. */
  const setType = (type) => apply((draft) => {
    draft.rule = { type, skipMonths: draft.rule.skipMonths ?? [], skipDates: draft.rule.skipDates ?? [] };
  });

  const toggleMonth = (m) => apply((draft) => {
    const months = new Set(draft.rule.skipMonths ?? []);
