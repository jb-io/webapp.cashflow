...
import { sortPhases } from '../../../core/amounts.ts';
import { today } from '../../../core/dateUtils.ts';
import { IconTrash } from '../Icons.tsx';
import type { AmountPhase, Entry } from '../../../core/types.ts';

/** Ändert genau diesen Eintrag; die Kopie besorgt der Store. */
export type ApplyToEntry = (mutate: (draft: Entry) => void) => void;

interface Props { entry: Entry; apply: ApplyToEntry; }

export default function AmountEditor({ entry, apply }: Props) {
  const phases = sortPhases(entry.amounts ?? []);

  const setPhases = (next: AmountPhase[]) => apply((draft) => { draft.amounts = sortPhases(next); });
