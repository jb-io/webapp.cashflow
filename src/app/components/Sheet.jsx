...
export default function Sheet({ anchorSelector, onClose, children }) {
  ...
  useEffect(() => {
    ...
    const onDown = (event) => {
      ...
      if (anchorSelector && event.target.closest?.(anchorSelector)) return;
      onClose();
    };
  ...
  useEffect(() => {
    const onKey = (event) => {
      ...
      onClose();
      document.querySelector(anchorSelector)?.focus();
    };
