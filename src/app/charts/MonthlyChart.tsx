...
import { rootIdOf, pathOf } from '../../core/categories.ts';
import { money, moneyShort, monthTick, NO_CATEGORY_COLOR } from '../format.ts';
import { chartTheme, useThemeTick } from './theme.ts';
import type { Category, MonthBucket } from '../../core/types.ts';

/** Eine Reihe im Stapel — Kategorie oder gebündelte „Übrige". */
...
/** Gebündelt und nach Gewicht sortiert. */
export function monthlySeries(
  months: MonthBucket[], categories: Category[], groupByRoot: boolean,
): Series[] {
  ...
  months.forEach((month, index) => {
    for (const [key, value] of Object.entries(month.byCategory)) {
      ...
      if (!sums.has(bucket)) sums.set(bucket, new Array<number>(months.length).fill(0));
      sums.get(bucket)![index] += value;
    }
  });

  const series: Series[] = [...sums.entries()].map(([key, values]) => {
...
export default function MonthlyChart({ months, categories, groupByRoot }: Props) {
  ...
  return (
    <>
      <div className="chart-box">
        {/* siehe BalanceChart: die Optionen sind bewusst nicht strukturell typisiert */}
