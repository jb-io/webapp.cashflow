...
import { day, money, moneyShort } from '../format.js';
import { chartTheme, useThemeTick, withAlpha } from './theme.js';

Chart.register(CategoryScale, LinearScale, PointElement, LineElement, Filler, Tooltip);

export default function BalanceChart({ forecast, stats }) {
  ...
  const { data, options } = useMemo(() => {
    ...
    const isMin = (p) => p.date === stats.min.date && p.balance === stats.min.balance;
    const isMax = (p) => p.date === stats.max.date && p.balance === stats.max.balance;
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            padding: 10,
            displayColors: false,
            callbacks: { label: (item) => `Saldo ${money(item.parsed.y)}` },
          },
        },
        scales: {
          x: {
