...
import { amountAt, signedAmountAt, setAmountAt } from '../src/core/amounts.js';
...
test('unsortierte Phasen werden korrekt ausgewertet', () => {
  const unsorted = { direction: 'income', amounts: [{ from: '2027-01-01', value: 2 }, { from: null, value: 1 }] };
  assert.equal(amountAt(unsorted, '2026-05-01'), 1);
  assert.equal(amountAt(unsorted, '2027-05-01'), 2);
});

test('vor der ersten Phase gilt 0', () => {
  const later = { direction: 'income', amounts: [{ from: '2027-01-01', value: 500 }] };
...
test('Vorzeichen kommt aus der Richtung, nicht aus dem gepflegten Wert', () => {
  ...
  assert.equal(signedAmountAt({ direction: 'expense', amounts: [{ from: null, value: -50 }] }, '2026-01-01'), -50);
});

test('setAmountAt ändert genau die heute gültige Phase', () => {
  const phased = { direction: 'expense', amounts: [
    { from: null, value: 820 },
    { from: '2027-01-01', value: 890 },
    { from: '2028-06-15', value: 940 },
  ] };
...
test('setAmountAt normalisiert das Vorzeichen und lässt das Original unberührt', () => {
  const entry = { direction: 'expense', amounts: [{ from: null, value: 100 }] };
...
test('setAmountAt vor der ersten Phase ändert die früheste', () => {
  const entry = { direction: 'income', amounts: [{ from: '2027-01-01', value: 500 }] };
...
test('setAmountAt auf einem Eintrag ohne Phasen legt eine an', () => {
  assert.deepEqual(setAmountAt({ direction: 'expense', amounts: [] }, '2026-01-01', 42), [{ from: null, value: 42 }]);
});
