...
import { buildForecast, aggregateByMonth } from '../src/core/forecast.ts';
import { computeStats } from '../src/core/stats.ts';
import { normalizeState } from '../src/core/model.ts';
import type { CashflowDoc, Entry } from '../src/core/types.ts';

function state(entries: Entry[], startBalance = 1000): CashflowDoc {
  return {
    version: 1,
    account: { name: 'Test', startBalance, startDate: '2026-01-01' },
    settings: { view: { from: '2026-01', to: '2026-12' } },
    categories: [{ id: 'c1', name: 'Wohnen', color: '#000000', parentId: null }],
    entries,
  };
}

const miete: Entry = {
  id: 'e1', name: 'Miete', categoryId: 'c1', direction: 'expense', note: '', active: true,
  startDate: '2026-01-01', endDate: null,
  rule: { type: 'monthly', dayOfMonth: 1, skipMonths: [12] },
  amounts: [{ from: null, value: 100 }],
};
const gehalt: Entry = {
  id: 'e2', name: 'Gehalt', categoryId: null, direction: 'income', note: '', active: true,
  startDate: '2026-01-01', endDate: null,
...
test('bei gleichem Datum kommt die Einnahme vor der Ausgabe', () => {
  const sameDay: Entry = { ...gehalt, id: 'e3', rule: { type: 'monthly', dayOfMonth: 1 } };
