...
import { expandRule, describeRule } from '../src/core/recurrence.ts';
import type { ISODate, Rule } from '../src/core/types.ts';

const win = (from: ISODate, to: ISODate) => ({ startDate: from, from, to });
...
test('unbekannter Regeltyp fällt auf', () => {
  // bewusst am Typ vorbei: so kommt es aus einer fremden Datei
  const kaputt = { type: 'quatsch' } as unknown as Rule;
  assert.throws(() => expandRule(kaputt, win('2026-01-01', '2026-12-31')), /Unbekannter Regeltyp/);
});
