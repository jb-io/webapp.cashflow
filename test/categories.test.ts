...
import {
  normalizeState, duplicateEntry, copyNameFor, createCategory, createEntry,
} from '../src/core/model.ts';
import type { Category, Entry } from '../src/core/types.ts';
...
test('duplicateEntry erzeugt eine unabhängige Kopie mit neuer id', () => {
  const original: Entry = {
    id: 'ent_1', name: 'Miete', categoryId: 'a', direction: 'expense', note: '', active: true,
...
test('Kopien bekommen eindeutige Namen', () => {
  const miete = createEntry({ name: 'Miete' });
  const entries: { name: string }[] = [miete];
  const first = duplicateEntry(miete, entries);
  entries.push(first);
  const second = duplicateEntry(miete, entries);
  entries.push(second);
